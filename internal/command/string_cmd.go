...
func getString(db types.Database, key string) (*data.SimpleString, bool, resp.Reply) {
	...
	str, ok := entity.Data.(*data.SimpleString)
	if !ok {
...
func execIncrBy(db types.Database, args [][]byte) resp.Reply {
	...
	// 不存在则当作 0
	if !exists {
		str = data.NewStringFromBytes([]byte("0"))
...
func execAppend(db types.Database, args [][]byte) resp.Reply {
	...
	if !exists {
		str = data.NewStringFromBytes(appendVal)
		db.PutEntity(key, &types.DataEntity{Data: str})
...
func execSetNX(db types.Database, args [][]byte) resp.Reply {
	...
	str := data.NewStringFromBytes(value)
	db.PutEntity(key, &types.DataEntity{Data: str})
	db.DeleteTTL(key)
...
func execMSet(db types.Database, args [][]byte) resp.Reply {
	...
	for i := 0; i < len(args); i += 2 {
		...
		db.PutEntity(key, &types.DataEntity{
			Data: data.NewStringFromBytes(val),
		})
		db.DeleteTTL(key)
	}
